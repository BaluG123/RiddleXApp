...
const MultiplicationScreen = ({ navigation }) => {
  ...
  const loadStats = async () => {
    try {
      const stats = await AsyncStorage.getItem('multiplicationStats');
      if (stats) {
  ...
  const saveStats = async (newScore, isCorrect) => {
    try {
      ...
      await AsyncStorage.setItem('multiplicationStats', JSON.stringify(newStats));
      ...
      parsedGlobalStats.categoryPerformance.multiplication.total += 1;
      if (isCorrect) {
        parsedGlobalStats.categoryPerformance.multiplication.correct += 1;
      }
  ...
  const generateProblem = () => {
    ...
    const maxNum = Math.min(10 + Math.floor(score / 100) * 2, 12);
    let num1 = Math.floor(Math.random() * maxNum) + 1;
    let num2 = Math.floor(Math.random() * maxNum) + 1;
    
    const answer = num1 * num2;

    // Generate wrong answers that are common multiplication mistakes
    const wrongAnswers = [
      answer + num1, // Common mistake: adding instead of multiplying
      answer - num2, // Another common mistake
      (num1 + 1) * num2, // Off-by-one error
    ];

    const options = [...wrongAnswers, answer]
      .sort(() => Math.random() - 0.5);

    setProblem({ num1, num2, answer, options });
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Multiplication Challenge</Text>
      </View>
