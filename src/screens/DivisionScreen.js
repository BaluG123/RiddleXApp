...
const DivisionScreen = ({ navigation }) => {
  ...
  const loadStats = useCallback(async () => {
    try {
      const stats = await AsyncStorage.getItem('divisionStats');
  ...
  const saveStats = useCallback(async (newScore, isCorrect) => {
    try {
      ...
      await AsyncStorage.setItem('divisionStats', JSON.stringify(newStats));
      ...
      parsedGlobalStats.categoryPerformance.division.total += 1;
      if (isCorrect) {
        parsedGlobalStats.categoryPerformance.division.correct += 1;
      }
  ...
  const generateProblem = useCallback(() => {
    ...
    try {
      ...
      const maxDivisor = Math.min(10 + Math.floor(score / 100), 12);
      const divisor = Math.floor(Math.random() * (maxDivisor - 1)) + 2;
      
      // Generate dividend that ensures whole number result
      ...
      if (isMounted.current) {
        setProblem({ dividend, divisor, answer, options });
      }
    } catch (error) {
