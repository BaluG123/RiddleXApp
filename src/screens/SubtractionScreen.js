...
const SubtractionScreen = ({ navigation }) => {
  ...
  const loadStats = async () => {
    try {
      const stats = await AsyncStorage.getItem('subtractionStats');
      if (stats) {
  ...
  const saveStats = async (newScore, isCorrect) => {
    try {
      ...
      await AsyncStorage.setItem('subtractionStats', JSON.stringify(newStats));
      ...
      parsedGlobalStats.categoryPerformance.subtraction.total += 1;
      if (isCorrect) {
        parsedGlobalStats.categoryPerformance.subtraction.correct += 1;
      }
  ...
  const generateProblem = () => {
    const maxNum = Math.min(20 + Math.floor(score / 50) * 10, 100);
    let num1 = Math.floor(Math.random() * maxNum) + 1;
    let num2 = Math.floor(Math.random() * maxNum) + 1;
    
    // Ensure num1 is larger to avoid negative results
    if (num2 > num1) {
      [num1, num2] = [num2, num1];
    }
    ...
    // Generate wrong answers that make sense for subtraction
    const wrongAnswers = [
      answer + Math.floor(Math.random() * 5) + 1,
      answer - Math.floor(Math.random() * 5) - 1,
      num1 - Math.floor(num2 / 2), // Common mistake
    ];
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Subtraction Challenge</Text>
      </View>
